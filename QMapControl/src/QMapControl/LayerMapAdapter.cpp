...
namespace qmapcontrol
{
    ...
    bool LayerMapAdapter::mousePressEvent(const QMouseEvent* /*mouse_event*/,
                               ...
                               const int& controller_zoom) const
    {
        ...
        if(isVisible(controller_zoom) && m_mapadapter != nullptr)
        {
            ...
            {
                // A unique ID is needed here, we simply use `this` pointer;
                void* unique_id = reinterpret_cast<void*>(const_cast<LayerMapAdapter*>(this));
                        ...
                        if(m_mapadapter->isTileValid(i, j, controller_zoom))
                        {
                            ...
                            painter.drawPixmap(top_left_px.rawPoint(),
                                               ImageManager::get().getImage(
                                                   ...
                                                   ImageLoadingPriority::Buffer,
                                                   unique_id));
                        }
                    }
                }

                RectWorldPx display_zone = backbuffer_rect_px;
                double horizontal_center = (display_zone.leftPx() + display_zone.rightPx()) / 2,
                       vertical_center = (display_zone.topPx() + display_zone.bottomPx()) / 2,
                       horizontal_length = display_zone.rightPx() - display_zone.leftPx(),
                       vertical_length = display_zone.topPx() - display_zone.bottomPx(),
                       display_left = horizontal_center - horizontal_length / 4,
                       display_right = horizontal_center + horizontal_length / 4,
                       display_top = vertical_center + vertical_length / 4,
                    display_bottom = vertical_center - vertical_length / 4;

                const int display_tile_left = display_left / tile_size_px.width(),
                          display_tile_right = display_right / tile_size_px.width(),
                          display_tile_top = display_top / tile_size_px.height(),
                    display_tile_bottom = display_bottom / tile_size_px.height();

                     // Loop through the tiles to draw (left to right).
                for(int i = display_tile_left; i <= display_tile_right; ++i)
                {
                    // Loop through the tiles to draw (top to bottom).
                    for(int j = display_tile_top; j <= display_tile_bottom; ++j)
                    {
                        // Check the tile is valid.
                        if(m_mapadapter->isTileValid(i, j, controller_zoom))
                        {
                            ImageManager::get().getImage(
                                m_mapadapter->tileQuery(i, j, controller_zoom),
                                ImageLoadingPriority::Display, unique_id);
                        }
                    }
                }
                ...
                for(int i = prefetch_tile_left; i <= prefetch_tile_right; ++i)
                {
                    ...
                    if(m_mapadapter->isTileValid(i, prefetch_tile_top, controller_zoom))
                    {
                        ...
                        ImageManager::get().getImage(
                            ...
                            ImageLoadingPriority::Prefetch, unique_id);
                    }
